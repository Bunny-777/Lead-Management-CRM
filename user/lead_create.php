<?php
// user/lead_create.php
$pageTitle = "Create Lead (User Panel)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($companyName) || empty($ownerName) || empty($mobile) || empty($officialEmail) || 
        ...
        try {
            ...
            // Auto assign lead to current logged in user
            $stmtLead->execute([
            ...
            $pdo->commit();
            $msg = "Lead record for '{$companyName}' created successfully!";
        } catch (Exception $e) {
            $pdo->rollBack();
...
<?php if ($msg): ?>
    <div class="alert alert-success">
        ...
        <a href="/CRM%20P/user/lead_list.php" style="margin-left: 12px; font-weight: 600; text-decoration: underline;">View My Leads &rarr;</a>
    </div>
<?php endif; ?>
...
<div class="alert alert-success" style="background-color: #eff6ff; border-color: #bfdbfe; color: #1e40af;">
    <strong>Note:</strong> Once you submit a lead, the primary details cannot be edited or deleted by users. You will be able to upload additional document attachments as needed.
</div>

<form action="" method="POST" enctype="multipart/form-data">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">1. Company & Owner Details</h3>
        </div>
        <div class="card-body">
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label" for="company_name">Company Name <span class="required">*</span></label>
                    <input type="text" id="company_name" name="company_name" class="form-control" placeholder="e.g., Global Traders Ltd" required>
                </div>

                <div class="form-group">
                    <label class="form-label" for="owner_name">Owner / Contact Name <span class="required">*</span></label>
                    <input type="text" id="owner_name" name="owner_name" class="form-control" placeholder="e.g., Sarah Smith" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="email" id="official_email" name="official_email" class="form-control" placeholder="info@globaltraders.com" required>
                </div>

                <div class="form-group">
                    <label class="form-label" for="personal_email">Personal Email</label>
                    <input type="email" id="personal_email" name="personal_email" class="form-control" placeholder="sarah.personal@gmail.com">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">2. Geography & Status</h3>
        </div>
        <div class="card-body">
            <div class="form-grid">
                <div class="form-group">
            ...
            <div class="form-group full-width" style="margin-top: 16px;">
                <label class="form-label" for="remarks">Remarks / Conversation Summary</label>
                <textarea id="remarks" name="remarks" class="form-control" placeholder="Add client requirement notes..."></textarea>
            </div>
        </div>
    </div>

    <!-- Dynamic Document Uploads Card (Up to 30) -->
    <div class="card">
        <div class="card-header">
            <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                <div>
                    <h3 class="card-title">3. Document Attachments (Up to 30 Files Supported)</h3>
                    <p style="font-size: 12px; color: var(--text-muted); margin-top: 2px;">Attach PDF, PNG, JPEG, DOCX, XLSX files</p>
                </div>
                <div style="display: flex; align-items: center; gap: 12px;">
                    <span id="doc-counter-label" class="badge badge-fresh">0 / 30 Attached</span>
                    <button type="button" id="btn-add-document" class="btn btn-sm btn-secondary">+ Add Document File</button>
                </div>
            </div>
        </div>
        <div class="card-body">
            <div id="documents-wrapper">
                <div class="doc-row" id="doc-row-initial">
                    <div>
                        <input type="text" name="doc_titles[]" class="form-control" placeholder="Document Title (e.g. Quotation, Proposal)">
                    </div>
                    <div>
    ...
    <div style="margin-bottom: 40px; text-align: right;">
        <a href="/CRM%20P/user/dashboard.php" class="btn btn-secondary" style="margin-right: 12px;">Cancel</a>
